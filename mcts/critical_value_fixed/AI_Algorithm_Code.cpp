...
struct mcts_edge * mcts_select_edge(struct mcts_node * node, enum turn turn){
	...
	for (int i = 0; i < 19; i++){
		for (int j = 0; j < 19; j++){
			...
			QU = e->Q + (C_PUCT*e->P*sqrt((double)total_n) / (1 + e->N));
			if (QU > best_QU){
				best_QU = QU;
				best_edge = e;
			}
...
void mcts_find_best_move(struct board * b, int * i1, int * j1, int * i2, int * j2, int cnt){
	...
	while (true){
		...
		time++;
		timeout = time >= 15000; //TODO: up to timelimit
		if (timeout){
			break;
		}
	}
...
void myturn(int cnt) {
	...
	if (cnt != 1){
		...
		for (int n = 0; n < opCnt; n++){
			board_put_enemy(&static_board, x[n], y[n]);
		}
	}

	mcts_find_best_move(&static_board, &(x[0]), &(y[0]), &(x[1]), &(y[1]), cnt);
	for (int n = 0; n < cnt; n++){
		board_put_player(&static_board, x[n], y[n]);
	}
