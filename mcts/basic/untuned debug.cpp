...
void mcts_find_best_move(struct board * b, int * i1, int * j1, int * i2, int * j2, int cnt){
	...
	while(true){
		...
		/*
		debug = false;
		if(depth > 0 && ilog[0] == 6 && jlog[0] == 2){
			//printf("hmmm\n");
		if(depth > 1 && ilog[1] == 7 && jlog[1] == 3){
			//printf("quite far\n");
		if(depth > 2 && ilog[2] == 13 && jlog[2] == 9){
			//printf("almost got it\n");
		if(depth > 3 && ilog[3] == 12 && jlog[3] == 8){
			printf("got it!, depth : %d\n", depth);
			printf("e: %p, e->result_node: %p\n", e, e->result_node);
			debug = true;
		}
		}
		}
		...
		*/
		...
		while(true){
			...
			/*
			if(e->pos_i == 10 && e->pos_j == 7 && turn == PLAYER_SECOND){
				//printf("l");
				printf("%p ", e->upper_edge);
				printf("|%d ", e->N);
				printf("%d ", tree.root->edges[10][7].N);
			}
			*/
			e->N ++;
			e->Q = e->W / e->N;
		...
		time++;
		timeout = time >= 9000; // has that weird score
		//timeout = time >= 7000;
		if(timeout){
			break;
		}
	...
	int i3, j3;
	max_n = 0;
	node = node->edges[*i2][*j2].result_node;
	if(node == NULL){
		printf("its null!!\n");
		fflush(0);
	}
	...
	e = &tree.root->edges[*i1][*j1];
	printf("Best (%d, %d) Q PF: %f\n", *i1, *j1, e->Q);

	mcts_display(b, e->result_node);
	e = &e->result_node->edges[*i2][*j2];
	printf("Best (%d, %d)  Q PS: %f\n", *i2, *j2, e->Q);

	mcts_display(b, e->result_node);
	e = &e->result_node->edges[i3][j3];
	printf("Best (%d, %d)  Q EF: %f N:%d\n", i3, j3, e->Q, e->N);
}
...
int main(){
	...
	board_put_player(&b, 1, 5);

	board_put_enemy(&b, 9, 7);
	board_put_enemy(&b, 9, 10);

	board_put_player(&b, 9, 8);
	board_put_player(&b, 4, 5);

	board_put_enemy(&b, 3, 5);
	board_put_enemy(&b, 5, 7);

	board_put_player(&b, 6, 5);
	board_put_player(&b, 8, 7);

	board_put_enemy(&b, 7, 6);
	board_put_enemy(&b, 4, 6);

	board_put_player(&b, 6, 8);
	board_put_player(&b, 2, 4);

	board_put_enemy(&b, 6, 6);
	board_put_enemy(&b, 7, 5);
